...
export const getTheTask = async (req, res, next) => {
    const taskId = req.params.taskId;
    try {
...
export const fetchTaskAssignedToProject = async (req, res, next) => {
    if (!req.user) {
        throw new Error('unauthorized to get all tasks');
    }
    const userId = req.user.id;
    try {
        ...
        if (projects.length > 0) {
            ...
            tasksByStatus.forEach(task => {
                if (task._id === 'To do') {
                    taskState.toDo = task.count;
                }
                if (task._id === 'In progress') {
                    taskState.inProgress = task.count;
                }
                if (task._id === 'Completed') {
                    taskState.completed = task.count;
                }
            });
...
export const getTaskState = async (req, res, next) => {
    if (!req.user) {
        throw new Error('unauthorized to get all tasks');
    }
    const userId = req.user.id;
    try {
        ...
        tasksByStatus.forEach((task) => {
            taskStatusCount[task._id] = task.count; //it's means that task._id will be "To do" , "In progress" or "Completed" and will contain the count
        });
        ...
        const taskWithNames = completedTasksByProject.map(task => {
            const project = projectIdName.find(p => p._id.equals(task._id));
